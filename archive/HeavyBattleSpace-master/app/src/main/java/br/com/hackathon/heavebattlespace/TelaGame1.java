...
import br.com.hackathon.heavebattlespace.AndGraph.AGSoundEffect;
...
public class TelaGame1 extends AGScene {
    /*******************************************
     * Name: CAGScene()
    ...
    public TelaGame1(AGGameManager pManager) {
        super(pManager);
    }
    @Override
    public void init() {
        cenario = this.createSprite(R.mipmap.cenario1, 1, 1);
        jogador = this.createSprite(R.mipmap.player1, 1, 1);
        explosao = this.createSprite(R.mipmap.explosao, 13, 1);


        cenario.setScreenPercent(100, 100);
        jogador.setScreenPercent(20, 12);
        explosao.setScreenPercent(15,15);
        ...
        int randomgeraMeteoro = 3000 + (int)(Math.random() * (4000 - 3000));
    ...
    public void moveNave() {
       jogador.vrPosition.setX(jogador.vrPosition.getX() + AGInputManager.vrAccelerometer.getAccelX()*7);
    }
    ...
    public void geraMeteoro (){
        ...
        if (tempoGeraMeteoro.isTimeEnded()) {
            meteoro = createSprite(R.mipmap.inimigolvl1, 1, 1);
        ...
        for (AGSprite m : listaMeteoros) {
            for (AGSprite t : listaMisseis) {
                if (t.collide(m) && t.bVisible && m.bVisible) {
                    ...
                    m.bVisible = false;
                    t.bVisible = false;
//                    explosao.vrPosition.setXY(m.vrPosition.getX(), m.vrPosition.getY());
//                    explosao.addAnimation(13, false,0, 13);
                }
            }
        }
