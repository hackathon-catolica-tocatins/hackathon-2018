...
public class TelaPerdeu extends AGScene{
    /*******************************************
     * Name: CAGScene()
    ...
    public TelaPerdeu(AGGameManager pManager) {
        super(pManager);
    }

    @Override
    public void init() {
        cenario = this.createSprite(R.mipmap.gameover, 1, 1);
        cenario.setScreenPercent(100, 100);
        cenario.vrPosition.setXY(AGScreenManager.iScreenWidth / 2, AGScreenManager.iScreenHeight / 2);

        AGSoundManager.vrMusic.loadMusic("loser.mp3",false);
